...
public class ApprovalLogic {
    ...
    public void submit(String type,long targetId,String applicantId){
        ...
        try(Connection con=db.getConnection()){con.setAutoCommit(false);try{
            ...
            applyRoute(con,approvalId,type);history(con,approvalId,"SUBMITTED",applicantId,null);new PortalLogic().notifyAdmins(con,"APPROVAL","新しい承認申請",("REPORT".equals(type)?"報告書":"勤怠")+"の申請が届きました。","/ReportApproval?id="+approvalId);con.commit();
    ...
    public void withdraw(long approvalId,String applicantId){DBBase db=new DBBase();try(Connection con=db.getConnection()){con.setAutoCommit(false);try{String type;long targetId;try(PreparedStatement ps=con.prepareStatement("SELECT application_type,target_id FROM approval_request WHERE approval_id=? AND applicant_id=? AND status='SUBMITTED' FOR UPDATE")){ps.setLong(1,approvalId);ps.setString(2,applicantId);try(ResultSet rs=ps.executeQuery()){if(!rs.next())throw new IllegalArgumentException("取下げできる申請がありません。");type=rs.getString(1);targetId=rs.getLong(2);}}try(PreparedStatement ps=con.prepareStatement("UPDATE approval_request SET status='WITHDRAWN',withdrawn_at=NOW() WHERE approval_id=?")){ps.setLong(1,approvalId);ps.executeUpdate();}String table="REPORT".equals(type)?"work_report":"attendance_record",id="REPORT".equals(type)?"report_id":"attendance_id",status="REPORT".equals(type)?"status":"approval_status";try(PreparedStatement ps=con.prepareStatement("UPDATE "+table+" SET "+status+"='DRAFT' WHERE "+id+"=?")){ps.setLong(1,targetId);ps.executeUpdate();}history(con,approvalId,"WITHDRAWN",applicantId,"申請者による取下げ");con.commit();}catch(Exception e){con.rollback();throw e;}}catch(IllegalArgumentException e){throw e;}catch(Exception e){throw new RuntimeException("申請の取下げに失敗しました。",e);}}
    public int reviewBatch(List<Long> ids,String reviewerId,String decision,String comment){int count=0;for(Long id:ids){review(id,reviewerId,decision,comment);count++;}return count;}
    public List<java.util.Map<String,Object>> history(long approvalId){DBBase db=new DBBase();try(Connection con=db.getConnection();PreparedStatement ps=con.prepareStatement("SELECT h.action,h.comment,h.acted_at,CONCAT(u.last_name,' ',u.first_name) acted_by FROM approval_history h JOIN users_info u ON u.user_id=h.acted_by_id WHERE h.approval_id=? ORDER BY h.acted_at")){ps.setLong(1,approvalId);try(ResultSet rs=ps.executeQuery()){List<java.util.Map<String,Object>> list=new ArrayList<>();while(rs.next()){java.util.Map<String,Object> m=new java.util.LinkedHashMap<>();m.put("action",rs.getString(1));m.put("comment",rs.getString(2));m.put("acted_at",rs.getTimestamp(3));m.put("acted_by",rs.getString(4));list.add(m);}return list;}}catch(SQLException e){throw new RuntimeException("承認履歴の取得に失敗しました。",e);}}
    public int submitAttendanceMonth(String applicantId,java.time.YearMonth month){
        ...
        try(Connection con=db.getConnection()){con.setAutoCommit(false);try{
            ...
            for(long targetId:targetIds){
                ...
                applyRoute(con,approvalId,"ATTENDANCE");history(con,approvalId,"SUBMITTED",applicantId,"月次一括申請（"+month+"）");
            }
            new PortalLogic().notifyAdmins(con,"APPROVAL","勤怠の月次申請",month+"の勤怠が"+targetIds.size()+"件申請されました。","/ReportApproval");con.commit();return targetIds.size();
    ...
    public void review(long approvalId,String reviewerId,String decision,String comment){
        ...
        try(Connection con=db.getConnection()){con.setAutoCommit(false);try{
            String type;long targetId;int step,required;try(PreparedStatement lock=con.prepareStatement("SELECT application_type,target_id,status,approval_step,required_steps FROM approval_request WHERE approval_id=? FOR UPDATE")){lock.setLong(1,approvalId);try(ResultSet rs=lock.executeQuery()){if(!rs.next()||!"SUBMITTED".equals(rs.getString("status")))throw new IllegalArgumentException("この申請は承認待ちではありません。");type=rs.getString("application_type");targetId=rs.getLong("target_id");step=rs.getInt("approval_step");required=rs.getInt("required_steps");}}
            if("APPROVED".equals(decision)&&step<required){try(PreparedStatement ps=con.prepareStatement("UPDATE approval_request SET approval_step=approval_step+1,reviewer_id=?,review_comment=? WHERE approval_id=?")){ps.setString(1,reviewerId);ps.setString(2,comment);ps.setLong(3,approvalId);ps.executeUpdate();}history(con,approvalId,"APPROVED",reviewerId,"段階 "+step+" / "+required+" 承認："+comment);new PortalLogic().notifyAdmins(con,"APPROVAL","次段階の承認依頼","次の承認段階へ進みました。","/ReportApproval?id="+approvalId);con.commit();return;}
            ...
            history(con,approvalId,decision,reviewerId,comment);new PortalLogic().notifyUser(con,applicant(con,approvalId),"APPROVAL","申請が処理されました",("APPROVED".equals(decision)?"承認されました":"差し戻されました")+(comment==null||comment.isBlank()?"":"："+comment),"/Portal");con.commit();
    ...
    private String applicant(Connection con,long id)throws SQLException{try(PreparedStatement ps=con.prepareStatement("SELECT applicant_id FROM approval_request WHERE approval_id=?")){ps.setLong(1,id);try(ResultSet rs=ps.executeQuery()){if(rs.next())return rs.getString(1);throw new SQLException("Applicant not found");}}}
    private void applyRoute(Connection con,long id,String type)throws SQLException{try(PreparedStatement ps=con.prepareStatement("UPDATE approval_request SET approval_step=1,required_steps=COALESCE((SELECT required_steps FROM approval_route_config WHERE application_type=?),1) WHERE approval_id=?")){ps.setString(1,type);ps.setLong(2,id);ps.executeUpdate();}}
    public void saveRoute(String type,int steps){if(!java.util.Set.of("REPORT","ATTENDANCE").contains(type)||steps<1||steps>5)throw new IllegalArgumentException("承認経路が不正です。");DBBase db=new DBBase();try(Connection c=db.getConnection();PreparedStatement p=c.prepareStatement("INSERT INTO approval_route_config(application_type,required_steps) VALUES(?,?) ON DUPLICATE KEY UPDATE required_steps=VALUES(required_steps)")){p.setString(1,type);p.setInt(2,steps);p.executeUpdate();}catch(SQLException e){throw new RuntimeException(e);}}
    public void addDelegate(String approver,String delegate,java.time.LocalDate from,java.time.LocalDate to){if(to.isBefore(from)||approver.equals(delegate))throw new IllegalArgumentException("代理承認期間が不正です。");DBBase db=new DBBase();try(Connection c=db.getConnection();PreparedStatement p=c.prepareStatement("INSERT INTO approval_delegate(approver_id,delegate_user_id,valid_from,valid_to) VALUES(?,?,?,?)")){p.setString(1,approver);p.setString(2,delegate);p.setObject(3,from);p.setObject(4,to);p.executeUpdate();}catch(SQLException e){throw new RuntimeException(e);}}
    public boolean canReview(String user,String permission){if("1".equals(permission))return true;DBBase db=new DBBase();try(Connection c=db.getConnection();PreparedStatement p=c.prepareStatement("SELECT 1 FROM approval_delegate WHERE delegate_user_id=? AND CURDATE() BETWEEN valid_from AND valid_to LIMIT 1")){p.setString(1,user);try(ResultSet r=p.executeQuery()){return r.next();}}catch(SQLException e){return false;}}
