...
public class AiResponseWorker implements Runnable {
    ...
    private Job claim() throws SQLException {
        ...
        try (Connection c = db.getConnection()) {
            ...
            try (PreparedStatement p = c.prepareStatement(
                    "SELECT j.job_id,j.character_id,j.source_type,j.conversation_id,j.source_message_id,j.requested_by_id,a.user_id,u.first_name,a.prompt_key,a.model_name FROM ai_response_job j JOIN ai_character a ON a.character_id=j.character_id JOIN users_info u ON u.user_id=a.user_id WHERE j.status='PENDING' AND a.active_flg='1' ORDER BY j.created_at LIMIT 1 FOR UPDATE")) {
    ...
    private void process(Job j) {
        try {
            Source s = source(j);
            String answer = client.respond(j.characterId, j.name, j.promptKey, j.model, j.type,
    ...
    private Source source(Job j) throws SQLException {
        ...
        try (Connection c = db.getConnection()) {
            ...
            String contextSql = "CHAT".equals(j.type)
                    ? "SELECT CONCAT(COALESCE(CONCAT(u.last_name,' ',u.first_name),m.posted_by_id),': ',m.message) line FROM chat_message m LEFT JOIN users_info u ON u.user_id=m.posted_by_id WHERE m.room_id=? AND m.message_id<? AND m.delete_flg='0' ORDER BY m.message_id DESC LIMIT 8"
                    : "SELECT line FROM (SELECT 0 sort_no,CONCAT('スレッド題名: ',title,'\n本文: ',thread_content) line FROM thread_info WHERE thread_id=? UNION ALL SELECT comment_id sort_no,CONCAT(author_id,': ',comment_text) line FROM thread_comment WHERE thread_id=? AND comment_id<?) x ORDER BY sort_no DESC LIMIT 8";
    ...
    private record Job(long id, long characterId, String type, String conversationId, long sourceMessageId,
            String requester, String aiUserId, String name, String promptKey, String model) {
    }
