...
public class AttendanceActionController extends HttpServlet {
        ...
        try (Connection c = db.getConnection()) {
            if ("clockIn".equals(action)) {
                try (PreparedStatement p = c.prepareStatement(
                        "INSERT INTO attendance_record(user_id,work_date,clock_in,work_type) SELECT ?,CURDATE(),CURTIME(),'OFFICE' WHERE NOT EXISTS(SELECT 1 FROM attendance_month_close WHERE user_id=? AND target_month=DATE_FORMAT(CURDATE(),'%Y-%m-01')) ON DUPLICATE KEY UPDATE clock_in=COALESCE(clock_in,CURTIME())")) {
                    p.setString(1, u.getUserId());
                    p.setString(2, u.getUserId());
                    p.executeUpdate();
                    AuditLogLogic.record(req, "ATTENDANCE", "CLOCK_IN", "USER", u.getUserId(), true, null);
                }
                ...
                if ("close".equals(action)) {
                    try (PreparedStatement p = c.prepareStatement(
                        ...
                        p.setString(1, u.getUserId());
                        p.setObject(2, month.atDay(1));
                        p.setString(3, u.getUserId());
                        p.executeUpdate();
                    }
                } else
                    try (PreparedStatement p = c.prepareStatement(
                            "DELETE FROM attendance_month_close WHERE user_id=? AND target_month=?")) {
                        p.setString(1, u.getUserId());
                        p.setObject(2, month.atDay(1));
                        p.executeUpdate();
                    }
                AuditLogLogic.record(req, "ATTENDANCE", "close".equals(action) ? "MONTH_CLOSED" : "MONTH_REOPENED",
                        "USER", u.getUserId(), true, month.toString());
            }
            req.getSession().setAttribute("attendanceFlash", "処理が完了しました。");
            req.getSession().setAttribute("attendanceFlashType", "success");
        } catch (Exception e) {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        YearMonth m;
        try {
            m = YearMonth.parse(req.getParameter("month"));
        } catch (Exception e) {
            m = YearMonth.now();
        }
        ...
        w.write('\ufeff');
        w.println("日付,勤務区分,出勤,退勤,休憩(分),残業(分),承認状態,備考");
        DBBase db = new DBBase();
        try (Connection c = db.getConnection();
            ...
            try (ResultSet r = p.executeQuery()) {
                while (r.next())
                    w.println(csv(r.getString(1)) + "," + csv(r.getString(2)) + "," + csv(r.getString(3)) + ","
                            + csv(r.getString(4)) + "," + r.getInt(5) + "," + r.getInt(6) + "," + csv(r.getString(7))
                            + "," + csv(r.getString(8)));
            }
