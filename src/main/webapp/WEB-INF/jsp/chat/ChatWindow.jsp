<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" import="jp.nw.entity.UserEntity,jp.nw.entity.ChatMessageEntity,java.util.List,jp.nw.util.DateFormatUtil" %>
...
<html lang="ja">
    ...
    <body class="chat-page">
        ...
        <div id="container" class="chat-container">
            <h1 class="page-title"><c:out value="${RoomName}" /></h1>
            <c:if test="${canManageMembers}"
                ><a
                    class="member-manage-link"
                    href="${pageContext.request.contextPath}/ChatMemberManage?roomId=${RoomId}"
                    >メンバーを招待</a
                ></c:if
            >
                    ...
                    <c:choose>
                        <c:when test="${message.postedById == loginUser.userId}">
                            <div class="chat-message my-message">
                                ...
                                <span class="message-time"> ${DateFormatUtil.format(message.createdAt)} </span>
                            </div>
                        </c:when>
                        ...
                        <c:otherwise>
                            <div class="chat-message other-message">
                                <div class="message-user">
                                    <c:out value="${message.postedByName}" />
                                    <c:if test="${message.postedByAccountType == 'AI'}"
            ...
            <div class="attachments">
                ...
                >
                    <input type="hidden" name="csrfToken" value="${sessionScope.attachmentCsrfToken}" /><input
                        type="hidden"
                        name="ownerType"
                        value="CHAT"
                    /><input type="hidden" name="ownerId" value="${RoomId}" /><input
                        type="file"
                        name="file"
